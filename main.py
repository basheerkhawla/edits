    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("SELECT * FROM bot_users WHERE tg_id=?", (tg_id,)) as cursor:
            ...
            if not row:
                ...
                await db.commit()
                bot_user = {"tg_id": tg_id, "language": lang, "state": "idle"}
            else:
                bot_user = dict(row)
    ...
    if "callback_query" in update:
        ...
        # ── Referrals ──
            ...
            if tg_username:
                async with aiosqlite.connect(DB_PATH) as db:
                    db.row_factory = aiosqlite.Row
                    async with db.execute("SELECT * FROM users WHERE telegram_id=?", (f"@{tg_username}",)) as cur:
                        row = await cur.fetchone()
                    if row:
                        site_user = dict(row)
            if not site_user:
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "no_referral_account"), bot_menu_keyboard(lang))
                return {"ok": True}
            # Get stats
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT COUNT(*) AS cnt FROM users WHERE referred_by=?", (site_user["referral_code"],)) as cur:
                    ref_count = (await cur.fetchone())["cnt"]
                async with db.execute("SELECT COALESCE(SUM(usd_amount),0) AS total FROM referral_earnings WHERE user_id=?", (site_user["id"],)) as cur:
                    earnings = (await cur.fetchone())["total"]
            balance = site_user.get("usd_balance", 0) or 0
            link = f"{SITE_URL}/?ref={site_user['referral_code']}"
            txt = t_bot(lang, "referral_info").replace("{link}", link).replace("{count}", str(ref_count)).replace("{earnings}", f"{earnings:.2f}").replace("{balance}", f"{balance:.2f}")
            kb = {"inline_keyboard": [
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                if method == "ton":
                    ...
                    cur = await db.execute(
                        ...
                        "ton_amount,ton_amount_nano,comment,expires_at,created_at,bot_chat_id,bot_lang) "
                        "VALUES (?,?,?,'awaiting_payment','ton_transfer',?,?,?,?,?,?,?)",
                        (target, stars, price, ton_amount, ton_nano_val, comment_code, expires, now, chat_id, lang)
                    )
                else:
                    cur = await db.execute(
                        "INSERT INTO orders (user_id,stars_amount,price_usd,status,payment_method,created_at,bot_chat_id,bot_lang) "
                        "VALUES (?,?,?,'pending',?,?,?,?)",
                        (target, stars, price, "cryptopay", now, chat_id, lang)
                    )
                await db.commit()
        ...
        if state == "waiting_custom":
            ...
            if tg_username:
                async with aiosqlite.connect(DB_PATH) as db:
                    ...
                    if row:
                        site_user = dict(row)
            if not site_user or (site_user.get("usd_balance", 0) or 0) < 2.0:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute("UPDATE users SET usd_balance = 0 WHERE id=?", (site_user["id"],))
                await db.execute(
                    ...
                    (site_user["id"], stars_amount, target, "stars", "", balance, "pending", now_iso))
                await db.commit()
            await notify_owner(f"💸 سحب من البوت (Stars): {balance}$ ({target}) من {site_user['email']}")
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                await db.execute(
                    "INSERT INTO withdrawals (user_id,stars_amount,recipient_username,method,wallet_address,usd_amount,status,created_at) VALUES (?,?,?,?,?,?,?,?)",
                    (site_user["id"], 0, "", "ton", wallet, balance, "pending", now_iso))
                await db.commit()
            await notify_owner(f"💸 سحب من البوت (TON): {balance}$ ({wallet}) من {site_user['email']}")
