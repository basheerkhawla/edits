...
ADMIN_SECRET          = os.getenv("ADMIN_SECRET", "") or secrets.token_hex(32)
if not os.getenv("JWT_SECRET"):
    logging.getLogger("starshub").warning("JWT_SECRET not set in .env - temporary random key used (sessions reset on restart)")
if not os.getenv("ADMIN_SECRET"):
    logging.getLogger("starshub").warning("ADMIN_SECRET not set in .env - temporary random key used")
...
BOT_USERNAME          = ""
SUPPORT_USERNAME      = os.getenv("SUPPORT_USERNAME", "@Tt09x")
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for migration in [
            ...
            "ALTER TABLE orders ADD COLUMN bot_chat_id INTEGER",
            "ALTER TABLE orders ADD COLUMN bot_message_id INTEGER",
            "ALTER TABLE orders ADD COLUMN bot_lang TEXT",
            "ALTER TABLE users ADD COLUMN usd_balance REAL DEFAULT 0.0",
            "ALTER TABLE referral_earnings ADD COLUMN usd_amount REAL DEFAULT 0.0",
            "ALTER TABLE referral_earnings ADD COLUMN price_at_time REAL DEFAULT 0.0",
            "ALTER TABLE withdrawals ADD COLUMN method TEXT DEFAULT 'stars'",
            "ALTER TABLE withdrawals ADD COLUMN wallet_address TEXT",
            "ALTER TABLE withdrawals ADD COLUMN usd_amount REAL DEFAULT 0.0",
            "ALTER TABLE bot_users ADD COLUMN temp_stars INTEGER",
            "ALTER TABLE bot_users ADD COLUMN temp_username TEXT",
            "ALTER TABLE bot_users ADD COLUMN temp_method TEXT",
            # ─── حقول الإحالات على مستوى البوت ─────────────────────
            "ALTER TABLE bot_users ADD COLUMN ref_code TEXT",
            "ALTER TABLE bot_users ADD COLUMN referred_by TEXT",
            "ALTER TABLE bot_users ADD COLUMN usd_balance REAL DEFAULT 0.0",
            "ALTER TABLE bot_users ADD COLUMN total_earned REAL DEFAULT 0.0",
            "ALTER TABLE bot_users ADD COLUMN ref_count INTEGER DEFAULT 0",
            "ALTER TABLE bot_users ADD COLUMN created_at TEXT",
            # ─── ربط الطلب بمُحيل من البوت ────────────────────────
            "ALTER TABLE orders ADD COLUMN bot_referrer_tg_id INTEGER",
            "ALTER TABLE orders ADD COLUMN user_id_tg INTEGER",  # tg_id للمشتري (للتمييز بين طلبات الموقع والبوت)
        ]:
            try:
        ...
        for idx in [
            ...
            "CREATE INDEX IF NOT EXISTS idx_bot_users_ref_code ON bot_users(ref_code)",
...
async def process_order_referral(order: dict):
    """يعالج عمولة الإحالة بعد نجاح الطلب — يدعم مسارَي البوت والموقع.
    - إذا الطلب من البوت وله bot_referrer_tg_id → يمنح مُحيل البوت.
    - وإلا (طلب موقع) → المسار القديم (referrer_code + account_id/pending).
    """
    if not order:
        return
    # مسار البوت
    if order.get("bot_referrer_tg_id"):
        try:
            await credit_bot_referrer(int(order["user_id_tg"]) if order.get("user_id_tg") else 0, order)
        except Exception as e:
            log.warning(f"process_order_referral (bot) error: {e}")
        return
    # المسار القديم للموقع
    if order.get("referrer_code"):
        if order.get("account_id"):
            await process_referral_commission(order)
        else:
            commission = calc_commission(order.get("stars_amount", 0) or 0)
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute(
                    ...
                    (order["referrer_code"], order["id"], commission,
                     order.get("user_id"), order.get("email"), datetime.utcnow().isoformat()),
                )
                await db.commit()
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[os.getenv("SITE_URL", "https://tele-starz.com")],
...
async def startup():
    await init_db()
    asyncio.create_task(refresh_ton_price())                       # جلب فوري عند الإقلاع
    ...
    if tg_token:
        TG_WEBHOOK_SECRET = hashlib.sha256(tg_token.encode()).hexdigest()[:16]
        tg_webhook_url = f"{SITE_URL}/tg-webhook/{TG_WEBHOOK_SECRET}"
        try:
            resp = req_lib.post(
                f"https://api.telegram.org/bot{tg_token}/setWebhook",
                json={"url": tg_webhook_url, "allowed_updates": ["message", "callback_query"]},
                timeout=10
            )
            data = resp.json()
            if data.get("ok"):
                ...
                log.error(f"❌ Telegram رفض تسجيل الـ webhook: {data} — البوت لن يستقبل الرسائل! تحقق من SITE_URL وشهادة HTTPS")
        except Exception as e:
            log.error(f"❌ فشل تسجيل Telegram Bot webhook: {e} — البوت لن يستقبل الرسائل!")
        # ── جلب اسم البوت ──
        try:
            me_resp = req_lib.get(f"https://api.telegram.org/bot{tg_token}/getMe", timeout=10)
            me_data = me_resp.json()
            if me_data.get("ok"):
                BOT_USERNAME = me_data["result"].get("username", "")
                log.info(f"🤖 Bot username: @{BOT_USERNAME}")
        except Exception as e:
            log.warning(f"⚠️ فشل جلب اسم البوت: {e}")
...
BOT_STRINGS = {
    "en": {
        ...
        "referral_info": "👥 *Your Referral Program*\n\n🔗 Your link:\n`{link}`\n\n👥 Referrals: *{count}*\n💰 Earnings: *${earnings}*\n💵 Balance: *${balance}*\n\n📊 You earn *10%* of profit from each purchase via your link!",
        "no_referral_account": "⚠️ No account found.\nRegister on our website with your Telegram username to access referrals.",
        "btn_withdraw": "💸 Withdraw",
        "withdraw_select": "💸 *Withdraw Earnings*\nBalance: *${balance}*\n\nChoose method:",
        "btn_withdraw_stars": "⭐ As Stars",
        "btn_withdraw_ton": "💎 As TON",
        "enter_wd_username": "Enter @username to receive stars:",
        "enter_wd_wallet": "Enter your TON wallet address:",
        "withdraw_min": "⚠️ Minimum $2.00. Balance: ${balance}",
        "withdraw_ok": "✅ Withdrawal submitted!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ Within 24h.",
        "withdraw_no_balance": "⚠️ Insufficient balance (${balance}).",
        "pay_confirmed": "✅ *Payment Confirmed!*\n⭐ {stars} stars → {username}\nThank you!",
        "pay_pending": "⏳ Waiting for payment...",
        "btn_check_pay": "🔍 Check Status",
        "order_creating": "⏳ Creating order...",
        "ton_expired": "⏰ Session expired. Please create a new order.",
        "pay_failed_bot": "❌ Payment failed or could not be completed. Please contact support."
    },
    "ar": {
        ...
        "referral_info": "👥 *برنامج الإحالة*\n\n🔗 رابطك:\n`{link}`\n\n👥 الإحالات: *{count}*\n💰 الأرباح: *${earnings}*\n💵 الرصيد: *${balance}*\n\n📊 تربح *10%* من أرباح كل عملية شراء عبر رابطك!",
        "no_referral_account": "⚠️ ليس لديك حساب.\nسجّل في موقعنا بمعرّف تيليجرام للوصول لبرنامج الإحالة.",
        "btn_withdraw": "💸 سحب",
        "withdraw_select": "💸 *سحب الأرباح*\nالرصيد: *${balance}*\n\nاختر الطريقة:",
        "btn_withdraw_stars": "⭐ نجوم",
        "btn_withdraw_ton": "💎 TON",
        "enter_wd_username": "أدخل @username لاستقبال النجوم:",
        "enter_wd_wallet": "أدخل عنوان محفظة TON:",
        "withdraw_min": "⚠️ الحد الأدنى 2$. رصيدك: ${balance}",
        "withdraw_ok": "✅ تم إرسال طلب السحب!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ خلال 24 ساعة.",
        "withdraw_no_balance": "⚠️ رصيد غير كافٍ (${balance}).",
        "pay_confirmed": "✅ *تم تأكيد الدفع!*\n⭐ {stars} نجمة → {username}\nشكراً لك!",
        "pay_pending": "⏳ في انتظار الدفع...",
        "btn_check_pay": "🔍 تحقق من الحالة",
        "order_creating": "⏳ جارٍ إنشاء الطلب...",
        "ton_expired": "⏰ انتهت صلاحية الجلسة. الرجاء إنشاء طلب جديد.",
        "pay_failed_bot": "❌ فشل الدفع أو لم يكتمل. الرجاء التواصل مع الدعم."
    },
    "ru": {
        ...
        "referral_info": "👥 *Реферальная программа*\n\n🔗 Ваша ссылка:\n`{link}`\n\n👥 Рефералов: *{count}*\n💰 Заработок: *${earnings}*\n💵 Баланс: *${balance}*\n\n📊 Вы получаете *10%* прибыли от каждой покупки по вашей ссылке!",
        "no_referral_account": "⚠️ Аккаунт не найден.\nЗарегистрируйтесь на сайте с вашим Telegram для доступа к рефералам.",
        "btn_withdraw": "💸 Вывод",
        "withdraw_select": "💸 *Вывод средств*\nБаланс: *${balance}*\n\nВыберите способ:",
        "btn_withdraw_stars": "⭐ Звёздами",
        "btn_withdraw_ton": "💎 TON",
        "enter_wd_username": "Введите @username для получения звёзд:",
        "enter_wd_wallet": "Введите адрес TON кошелька:",
        "withdraw_min": "⚠️ Минимум $2.00. Баланс: ${balance}",
        "withdraw_ok": "✅ Заявка отправлена!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ В течение 24ч.",
        "withdraw_no_balance": "⚠️ Недостаточно средств (${balance}).",
        "pay_confirmed": "✅ *Оплата подтверждена!*\n⭐ {stars} звёзд → {username}\nСпасибо!",
        "pay_pending": "⏳ Ожидание оплаты...",
        "btn_check_pay": "🔍 Проверить",
    ...
    "fa": {
        ...
        "btn_referrals": "👥 ارجاع‌ها",
        "referral_info": "👥 *برنامه ارجاع*\n\n🔗 لینک شما:\n`{link}`\n\n👥 ارجاع‌ها: *{count}*\n💰 درآمد: *${earnings}*\n💵 موجودی: *${balance}*\n\n📊 شما *10%* سود از هر خرید از طریق لینک خود دریافت می‌کنید!",
        "no_referral_account": "⚠️ حساب یافت نشد.\nدر وبسایت ما ثبت‌نام کنید.",
        "btn_withdraw": "💸 برداشت",
        "withdraw_select": "💸 *برداشت*\nموجودی: *${balance}*\n\nروش را انتخاب کنید:",
        "btn_withdraw_stars": "⭐ ستاره",
        "btn_withdraw_ton": "💎 TON",
        "enter_wd_username": "@username را برای دریافت ستاره وارد کنید:",
        "enter_wd_wallet": "آدرس کیف پول TON را وارد کنید:",
        "withdraw_min": "⚠️ حداقل 2$. موجودی: ${balance}",
        "withdraw_ok": "✅ درخواست برداشت ارسال شد!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ ظرف 24 ساعت.",
        "withdraw_no_balance": "⚠️ موجودی ناکافی (${balance}).",
        "pay_confirmed": "✅ *پرداخت تأیید شد!*\n⭐ {stars} ستاره → {username}\nسپاسگزاریم!",
        "pay_pending": "⏳ در انتظار پرداخت...",
        "btn_check_pay": "🔍 بررسی",
        "order_creating": "⏳ ایجاد سفارش..."
    },
    "fr": {
        ...
        "referral_info": "👥 *Programme de parrainage*\n\n🔗 Votre lien:\n`{link}`\n\n👥 Parrainés: *{count}*\n💰 Gains: *${earnings}*\n💵 Solde: *${balance}*\n\n📊 Vous gagnez *10%* des bénéfices de chaque achat via votre lien!",
        "no_referral_account": "⚠️ Aucun compte trouvé.\nInscrivez-vous sur notre site.",
        "btn_withdraw": "💸 Retirer",
        "withdraw_select": "💸 *Retrait*\nSolde: *${balance}*\n\nChoisissez la méthode:",
        "btn_withdraw_stars": "⭐ En étoiles",
        "btn_withdraw_ton": "💎 En TON",
        "enter_wd_username": "Entrez @username pour recevoir les étoiles:",
        "enter_wd_wallet": "Entrez l'adresse de votre portefeuille TON:",
        "withdraw_min": "⚠️ Minimum 2$. Solde: ${balance}",
        "withdraw_ok": "✅ Demande envoyée!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ Sous 24h.",
        "withdraw_no_balance": "⚠️ Solde insuffisant (${balance}).",
        "pay_confirmed": "✅ *Paiement confirmé!*\n⭐ {stars} étoiles → {username}\nMerci!",
        "pay_pending": "⏳ En attente du paiement...",
        "btn_check_pay": "🔍 Vérifier",
        "order_creating": "⏳ Création de la commande..."
    },
    "de": {
        ...
        "referral_info": "👥 *Empfehlungsprogramm*\n\n🔗 Ihr Link:\n`{link}`\n\n👥 Empfehlungen: *{count}*\n💰 Verdienst: *${earnings}*\n💵 Guthaben: *${balance}*\n\n📊 Sie verdienen *10%* vom Gewinn jedes Kaufs über Ihren Link!",
        "no_referral_account": "⚠️ Kein Konto gefunden.\nRegistrieren Sie sich auf unserer Website.",
        "btn_withdraw": "💸 Auszahlung",
        "withdraw_select": "💸 *Auszahlung*\nGuthaben: *${balance}*\n\nMethode wählen:",
        "btn_withdraw_stars": "⭐ Als Sterne",
        "btn_withdraw_ton": "💎 Als TON",
        "enter_wd_username": "@username für Sterne-Empfang eingeben:",
        "enter_wd_wallet": "TON Wallet-Adresse eingeben:",
        "withdraw_min": "⚠️ Minimum $2.00. Guthaben: ${balance}",
        "withdraw_ok": "✅ Anfrage gesendet!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ Innerhalb 24h.",
        "withdraw_no_balance": "⚠️ Unzureichendes Guthaben (${balance}).",
        "pay_confirmed": "✅ *Zahlung bestätigt!*\n⭐ {stars} Sterne → {username}\nDanke!",
        "pay_pending": "⏳ Warte auf Zahlung...",
        "btn_check_pay": "🔍 Prüfen",
        "order_creating": "⏳ Bestellung wird erstellt..."
    },
    "es": {
        ...
        "referral_info": "👥 *Programa de Referidos*\n\n🔗 Tu enlace:\n`{link}`\n\n👥 Referidos: *{count}*\n💰 Ganancias: *${earnings}*\n💵 Saldo: *${balance}*\n\n📊 ¡Ganas el *10%* de las ganancias de cada compra con tu enlace!",
        "no_referral_account": "⚠️ Cuenta no encontrada.\nRegístrate en nuestro sitio web.",
        "btn_withdraw": "💸 Retirar",
        "withdraw_select": "💸 *Retirar ganancias*\nSaldo: *${balance}*\n\nElige método:",
        "btn_withdraw_stars": "⭐ En estrellas",
        "btn_withdraw_ton": "💎 En TON",
        "enter_wd_username": "Ingresa @username para recibir estrellas:",
        "enter_wd_wallet": "Ingresa la dirección de tu billetera TON:",
        "withdraw_min": "⚠️ Mínimo $2.00. Saldo: ${balance}",
        "withdraw_ok": "✅ ¡Solicitud enviada!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ En 24h.",
        "withdraw_no_balance": "⚠️ Saldo insuficiente (${balance}).",
        "pay_confirmed": "✅ *¡Pago confirmado!*\n⭐ {stars} estrellas → {username}\n¡Gracias!",
        "pay_pending": "⏳ Esperando pago...",
        "btn_check_pay": "🔍 Verificar",
        "order_creating": "⏳ Creando pedido..."
    },
    "tr": {
        ...
        "referral_info": "👥 *Referans Programı*\n\n🔗 Linkiniz:\n`{link}`\n\n👥 Referanslar: *{count}*\n💰 Kazanç: *${earnings}*\n💵 Bakiye: *${balance}*\n\n📊 Linkiniz üzerinden yapılan her alışverişten *%10* kazanırsınız!",
        "no_referral_account": "⚠️ Hesap bulunamadı.\nWeb sitemize kayıt olun.",
        "btn_withdraw": "💸 Çekim",
        "withdraw_select": "💸 *Kazanç Çekimi*\nBakiye: *${balance}*\n\nYöntem seçin:",
        "btn_withdraw_stars": "⭐ Yıldız olarak",
        "btn_withdraw_ton": "💎 TON olarak",
        "enter_wd_username": "Yıldız almak için @username girin:",
        "enter_wd_wallet": "TON cüzdan adresinizi girin:",
        "withdraw_min": "⚠️ Minimum $2.00. Bakiye: ${balance}",
        "withdraw_ok": "✅ Talep gönderildi!\n💰 ${amount}\n📦 {method}\n📍 {target}\n⏳ 24 saat içinde.",
        "withdraw_no_balance": "⚠️ Yetersiz bakiye (${balance}).",
        "pay_confirmed": "✅ *Ödeme onaylandı!*\n⭐ {stars} yıldız → {username}\nTeşekkürler!",
        "pay_pending": "⏳ Ödeme bekleniyor...",
        "btn_check_pay": "🔍 Kontrol Et",
...
def gen_bot_ref_code(tg_id: int) -> str:
    """يولد رمز إحالة قصير فريد لكل مستخدم بوت (مبني على tg_id + salt عشوائي قصير)."""
    # مثال: TG12345_XYZ4
    salt = "".join(secrets.choice(string.ascii_uppercase + string.digits) for _ in range(4))
    return f"TG{tg_id}{salt}"

async def get_bot_user(tg_id: int, tg_lang: str, tg_username: str = None) -> dict:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM bot_users WHERE tg_id=?", (tg_id,)) as cursor:
            ...
            if not row:
                ...
                ref_code = gen_bot_ref_code(tg_id)
                now_iso = datetime.utcnow().isoformat()
                await db.execute(
                    "INSERT INTO bot_users (tg_id, language, state, ref_code, usd_balance, total_earned, ref_count, created_at) VALUES (?, ?, 'idle', ?, 0.0, 0.0, 0, ?)",
                    (tg_id, lang, ref_code, now_iso)
                )
                await db.commit()
                bot_user = {"tg_id": tg_id, "language": lang, "state": "idle", "ref_code": ref_code,
                            "usd_balance": 0.0, "total_earned": 0.0, "ref_count": 0, "created_at": now_iso}
            else:
                bot_user = dict(row)
                # ضمان امتلاك ref_code (لمستخدمين قدامى)
                if not bot_user.get("ref_code"):
                    ref_code = gen_bot_ref_code(tg_id)
                    await db.execute("UPDATE bot_users SET ref_code=? WHERE tg_id=?", (ref_code, tg_id))
                    await db.commit()
                    bot_user["ref_code"] = ref_code
                
        if tg_username:
            username_at = f"@{tg_username}"
            async with db.execute("SELECT id FROM users WHERE telegram_id=?", (username_at,)) as cursor:
                main_user = await cursor.fetchone()
            if not main_user:
                email = f"bot_tg_{tg_id}@tele-starz.local"
                password = hash_password(secrets.token_hex(8))
                ref_code = gen_referral_code()
                now = datetime.utcnow().isoformat()
                try:
                    await db.execute(
                        "INSERT INTO users (email, password_hash, referral_code, telegram_id, stars_balance, created_at) VALUES (?, ?, ?, ?, 0, ?)",
                        (email, password, ref_code, username_at, now)
                    )
                    await db.commit()
                except Exception as e:
                    log.warning(f"Failed to auto-create user: {e}")
                    
        return bot_user

async def get_bot_user_by_ref_code(ref_code: str) -> Optional[dict]:
    """يبحث عن مستخدم بوت بواسطة رمز إحالته."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM bot_users WHERE ref_code=?", (ref_code,)) as cur:
            row = await cur.fetchone()
    return dict(row) if row else None

async def set_bot_referrer(tg_id: int, referrer_tg_id: int):
    """يربط مستخدم البوت بمُحيله (فقط إذا لم يكن مربوطاً مسبقاً وليس بنفسه)."""
    if tg_id == referrer_tg_id:
        return False
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT referred_by FROM bot_users WHERE tg_id=?", (tg_id,)) as cur:
            row = await cur.fetchone()
        if row and row["referred_by"]:
            return False  # مربوط مسبقاً
        # تحقق من وجود المُحيل
        async with db.execute("SELECT tg_id FROM bot_users WHERE tg_id=?", (referrer_tg_id,)) as cur:
            if not await cur.fetchone():
                return False
        await db.execute("UPDATE bot_users SET referred_by=? WHERE tg_id=?", (str(referrer_tg_id), tg_id))
        await db.execute("UPDATE bot_users SET ref_count = COALESCE(ref_count, 0) + 1 WHERE tg_id=?", (referrer_tg_id,))
        await db.commit()
    return True

async def credit_bot_referrer(buyer_tg_id: int, order: dict):
    """يمنح المُحيل (بوت) نسبته من ربح الطلب."""
    if not buyer_tg_id:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT referred_by FROM bot_users WHERE tg_id=?", (buyer_tg_id,)) as cur:
            row = await cur.fetchone()
        if not row or not row["referred_by"]:
            return
        try:
            referrer_tg_id = int(row["referred_by"])
        except (TypeError, ValueError):
            return
        # حساب العمولة على أساس صافي الربح
        price_usd = float(order.get("price_usd") or 0.0)
        stars_amount = int(order.get("stars_amount") or 0)
        cost_price = COST_PER_STAR * stars_amount
        profit_usd = max(0.0, price_usd - cost_price)
        comm_percent = float(GLOBAL_CONFIG.get("COMMISSION_PERCENT", 10.0))
        commission_usd = round(profit_usd * (comm_percent / 100.0), 4)
        if commission_usd <= 0:
            return
        # منع الازدواج
        async with db.execute(
            "SELECT id FROM referral_earnings WHERE order_id=? AND user_id=?",
            (order["id"], -referrer_tg_id)  # نستعمل قيمة سالبة لتمييز مُحيل البوت (ليست FK)
        ) as cur:
            if await cur.fetchone():
                return
        now = datetime.utcnow().isoformat()
        try:
            await db.execute(
                "INSERT INTO referral_earnings (user_id, order_id, stars_amount, usd_amount, price_at_time, created_at) VALUES (?,?,?,?,?,?)",
                (-referrer_tg_id, order["id"], stars_amount, commission_usd,
                 GLOBAL_CONFIG.get("PRICE_PER_STAR", 0.018), now),
            )
        except Exception as e:
            log.warning(f"credit_bot_referrer: insert referral_earnings failed: {e}")
        await db.execute(
            "UPDATE bot_users SET usd_balance = COALESCE(usd_balance,0) + ?, total_earned = COALESCE(total_earned,0) + ? WHERE tg_id=?",
            (commission_usd, commission_usd, referrer_tg_id),
        )
        await db.commit()
    # أبلغ المُحيل عبر البوت
    try:
        lang_row = None
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT language FROM bot_users WHERE tg_id=?", (referrer_tg_id,)) as cur:
                lang_row = await cur.fetchone()
        rlang = (dict(lang_row).get("language") if lang_row else "en") or "en"
        msg_map = {
            "ar": f"💰 *عمولة إحالة جديدة!*\n\nربحت *${commission_usd:.4f}* من عملية شراء أحد المُحالين.\n\n📊 استخدم زر *الإحالات* لمشاهدة رصيدك.",
            "en": f"💰 *New referral commission!*\n\nYou earned *${commission_usd:.4f}* from a referral's purchase.\n\n📊 Use the *Referrals* button to view your balance.",
            "ru": f"💰 *Новая реферальная комиссия!*\n\nВы заработали *${commission_usd:.4f}* с покупки реферала.\n\n📊 Нажмите *Рефералы*, чтобы увидеть баланс.",
            "fa": f"💰 *کمیسیون ارجاع جدید!*\n\nشما *${commission_usd:.4f}* از خرید یکی از ارجاع‌شدگان کسب کردید.\n\n📊 دکمه *ارجاع‌ها* را برای مشاهده موجودی بزنید.",
            "fr": f"💰 *Nouvelle commission de parrainage !*\n\nVous avez gagné *${commission_usd:.4f}* sur l'achat d'un parrainé.\n\n📊 Utilisez le bouton *Parrainages* pour voir votre solde.",
            "de": f"💰 *Neue Empfehlungsprovision!*\n\nSie haben *${commission_usd:.4f}* durch einen Kauf eines Empfohlenen verdient.\n\n📊 Verwenden Sie die *Empfehlungen*-Schaltfläche, um Ihren Saldo einzusehen.",
            "es": f"💰 *¡Nueva comisión de referido!*\n\nGanaste *${commission_usd:.4f}* por la compra de un referido.\n\n📊 Usa el botón *Referidos* para ver tu saldo.",
            "tr": f"💰 *Yeni referans komisyonu!*\n\nBir referansın alışverişinden *${commission_usd:.4f}* kazandınız.\n\n📊 Bakiyenizi görmek için *Referanslar* düğmesini kullanın.",
        }
        await send_tg_message(referrer_tg_id, msg_map.get(rlang, msg_map["en"]), bot_menu_keyboard(rlang))
    except Exception as e:
        log.warning(f"credit_bot_referrer notify error: {e}")
...
async def send_tg_message(chat_id: int, text: str, reply_markup: dict = None):
    ...
    try:
        r = await asyncio.to_thread(req_lib.post, f"https://api.telegram.org/bot{tg_token}/sendMessage", json=payload, timeout=10)
        if r.status_code != 200:
            # Retry without parse_mode في حال Markdown مكسور
            log.warning(f"Telegram sendMessage فشل ({r.status_code}): {r.text[:300]} — إعادة المحاولة بدون Markdown")
        ...
        log.warning(f"Telegram API send message error: {e}")
    return None

async def edit_tg_message(chat_id: int, message_id: int, text: str, reply_markup: dict = None):
    """يحرر الرسالة؛ عند فشل التحرير (message not modified / message can't be edited)
    يُرسل رسالة جديدة بدلاً من ذلك، حتى لا تتعطل واجهة البوت."""
    tg_token = os.getenv("TELEGRAM_BOT_TOKEN", "")
    if not tg_token:
        return
    ...
    if reply_markup:
        payload["reply_markup"] = reply_markup
    try:
        r = await asyncio.to_thread(req_lib.post, f"https://api.telegram.org/bot{tg_token}/editMessageText", json=payload, timeout=10)
        if r.status_code == 200:
            return
        # فحص نوع الخطأ
        try:
            err = r.json().get("description", "") or ""
        except Exception:
            err = r.text or ""
        err_low = err.lower()
        if "not modified" in err_low:
            # المحتوى نفسه → لا حاجة لإجراء
            return
        if "can't be edited" in err_low or "message to edit not found" in err_low or "message is not modified" in err_low:
            log.info(f"editMessageText → إرسال رسالة جديدة (السبب: {err[:120]})")
            await send_tg_message(chat_id, text, reply_markup)
            return
        # إعادة المحاولة بدون parse_mode
        log.warning(f"Telegram editMessageText فشل ({r.status_code}): {err[:200]} — محاولة بدون Markdown")
        payload.pop("parse_mode", None)
        try:
            r2 = await asyncio.to_thread(req_lib.post, f"https://api.telegram.org/bot{tg_token}/editMessageText", json=payload, timeout=10)
            if r2.status_code != 200:
                # آخر حل: أرسل رسالة جديدة
                await send_tg_message(chat_id, text, reply_markup)
        except Exception:
            await send_tg_message(chat_id, text, reply_markup)
    except Exception as e:
        log.warning(f"Telegram API edit message error: {e}")
        try:
            await send_tg_message(chat_id, text, reply_markup)
        except Exception:
            pass

async def answer_callback(callback_id: str, text: str = None, show_alert: bool = False):
    tg_token = os.getenv("TELEGRAM_BOT_TOKEN", "")
    if not tg_token or not callback_id:
        return
    payload = {"callback_query_id": callback_id}
    if text:
        # Telegram callback text limit is 200 chars
        payload["text"] = text[:200]
        if show_alert:
            payload["show_alert"] = True
    try:
        r = await asyncio.to_thread(req_lib.post, f"https://api.telegram.org/bot{tg_token}/answerCallbackQuery", json=payload, timeout=5)
        if r.status_code != 200:
            log.warning(f"answerCallbackQuery failed ({r.status_code}): {r.text[:200]}")
    except Exception as e:
        log.warning(f"Telegram API answer callback error: {e}")
...
def bot_menu_keyboard(lang: str):
    return {
        "inline_keyboard": [
            ...
            [
                {"text": t_bot(lang, "btn_referrals"), "callback_data": "referrals"}
            ]
        ]
    }
...
async def telegram_webhook(secret: str, update: dict = Body(...)):
    tg_token = os.getenv("TELEGRAM_BOT_TOKEN", "")
    if secret != hashlib.sha256(tg_token.encode()).hexdigest()[:16]:
        raise HTTPException(403, "Forbidden")
    
    try:
        return await _handle_tg_update(update)
    except Exception as e:
        log.exception(f"❌ خطأ داخلي في معالجة تحديث Telegram: {e}")
        # حاول الرد على callback query لكسر حالة التحميل عند المستخدم
        try:
            cb = update.get("callback_query")
            if cb and cb.get("id"):
                await answer_callback(cb["id"], "⚠️ حدث خطأ، حاول مجدداً", show_alert=False)
        except Exception:
            pass
        return {"ok": True}


async def _handle_tg_update(update: dict):
    if "callback_query" in update:
        ...
        user = await get_bot_user(tg_id, tg_lang, tg_username)
        lang = user["language"]
        
        # اُنهِ حالة "التحميل" فوراً لكل الأزرار — سنستخدم show_alert فقط عند الحاجة
        ...
        # ── Referrals (بوت-مرتكزة) ──
        elif data == "referrals":
            # اعتمد على بيانات bot_users بدل users في الموقع
            ref_code = user.get("ref_code") or gen_bot_ref_code(tg_id)
            # ضمان الحفظ في حال كان قديماً
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT COUNT(*) AS cnt FROM bot_users WHERE referred_by=?", (str(tg_id),)) as cur:
                    ref_count_row = await cur.fetchone()
                ref_count = ref_count_row["cnt"] if ref_count_row else 0
                async with db.execute("SELECT usd_balance, total_earned FROM bot_users WHERE tg_id=?", (tg_id,)) as cur:
                    stat_row = await cur.fetchone()
            balance = (stat_row["usd_balance"] if stat_row else 0) or 0
            earnings = (stat_row["total_earned"] if stat_row else 0) or 0
            
            # رابط الإحالة يشير للبوت مباشرة
            bot_user_name = BOT_USERNAME or "your_bot"
            link = f"https://t.me/{bot_user_name}?start=ref_{ref_code}"
            
            comm_percent = float(GLOBAL_CONFIG.get("COMMISSION_PERCENT", 10.0))
            # استعمل نص referral_info مع استبدال 10% بالنسبة الحقيقية
            base_txt = t_bot(lang, "referral_info")
            base_txt = base_txt.replace("10%", f"{comm_percent:.0f}%").replace("*10%*", f"*{comm_percent:.0f}%*")
            txt = (base_txt
                   .replace("{link}", link)
                   .replace("{count}", str(ref_count))
                   .replace("{earnings}", f"{earnings:.2f}")
            ...
            share_text_map = {
                "ar": f"⭐ اشترِ نجوم تيليجرام بأفضل سعر عبر هذا البوت!",
                "en": f"⭐ Buy Telegram Stars at the best price via this bot!",
                "ru": f"⭐ Покупайте звёзды Telegram по лучшей цене!",
                "fa": f"⭐ ستاره‌های تلگرام را با بهترین قیمت بخرید!",
                "fr": f"⭐ Achetez des étoiles Telegram au meilleur prix !",
                "de": f"⭐ Kaufen Sie Telegram-Sterne zum besten Preis!",
                "es": f"⭐ ¡Compra estrellas de Telegram al mejor precio!",
                "tr": f"⭐ Telegram Yıldızlarını en iyi fiyata satın alın!",
            }
            share_url = (
                "https://t.me/share/url?url=" + link
                + "&text=" + share_text_map.get(lang, share_text_map["en"]).replace(" ", "%20")
            )
            
            kb = {"inline_keyboard": [
                [{"text": share_labels.get(lang, share_labels["en"]), "url": share_url}],
                [{"text": t_bot(lang, "btn_withdraw"), "callback_data": "withdraw"}],
                [{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]
            ]}
            await edit_tg_message(chat_id, msg_id, txt, kb)

        elif data == "withdraw":
            # اعتمد رصيد البوت
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT usd_balance FROM bot_users WHERE tg_id=?", (tg_id,)) as cur:
                    row = await cur.fetchone()
            balance = (row["usd_balance"] if row else 0) or 0
            if balance < 2.0:
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "withdraw_min").replace("{balance}", f"{balance:.2f}"), bot_menu_keyboard(lang))
                return {"ok": True}
            txt = t_bot(lang, "withdraw_select").replace("{balance}", f"{balance:.2f}")
            kb = {"inline_keyboard": [
                [{"text": t_bot(lang, "btn_withdraw_stars"), "callback_data": "wd_stars"}],
                [{"text": t_bot(lang, "btn_withdraw_ton"), "callback_data": "wd_ton"}],
                [{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]
            ]}
            await edit_tg_message(chat_id, msg_id, txt, kb)

        elif data == "wd_stars":
            await set_bot_user_state(tg_id, "waiting_wd_username")
            await edit_tg_message(chat_id, msg_id, t_bot(lang, "enter_wd_username"), {"inline_keyboard": [[{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]]})

        elif data == "wd_ton":
            await set_bot_user_state(tg_id, "waiting_wd_wallet")
            await edit_tg_message(chat_id, msg_id, t_bot(lang, "enter_wd_wallet"), {"inline_keyboard": [[{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]]})

        # ── Check payment status callbacks ──
        elif data.startswith("chk_cp_"):
            oid = int(data.split("_")[2])
            res = await process_cryptopay_check(oid)
            order = await get_order_by_id(oid)
            if res.get("status") == "completed" or (order and order["status"] in ("completed", "paid")):
                await answer_callback(cb["id"], text=t_bot(lang, "pay_confirmed").split("\n")[0], show_alert=True)
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "pay_confirmed").replace("{stars}", str(order["stars_amount"])).replace("{username}", escape_md(order["user_id"])))
            elif res.get("status") == "expired" or (order and order["status"] == "expired"):
                await answer_callback(cb["id"], text="Expired", show_alert=True)
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "error_invoice"), bot_menu_keyboard(lang))
            else:
                await answer_callback(cb["id"], text=t_bot(lang, "pay_pending"), show_alert=True)

        elif data.startswith("chk_ton_"):
            oid = int(data.split("_")[2])
            res = await process_ton_check(oid)
            order = await get_order_by_id(oid)
            if res.get("status") == "completed" or (order and order["status"] in ("completed", "paid")):
                await answer_callback(cb["id"], text=t_bot(lang, "pay_confirmed").split("\n")[0], show_alert=True)
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "pay_confirmed").replace("{stars}", str(order["stars_amount"])).replace("{username}", escape_md(order["user_id"])))
                ...
                await answer_callback(cb["id"], text="Expired", show_alert=True)
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "ton_expired"), bot_menu_keyboard(lang))
            else:
                await answer_callback(cb["id"], text=t_bot(lang, "pay_pending"), show_alert=True)
        ...
        # ── Payment methods ──
            ...
            await edit_tg_message(chat_id, msg_id, t_bot(lang, "order_creating"))

            now = datetime.utcnow().isoformat()
            # هل لدى المشتري (بوت) مُحيل؟
            bot_referrer = user.get("referred_by")
            try:
                bot_referrer_tg_id = int(bot_referrer) if bot_referrer else None
            except (TypeError, ValueError):
                bot_referrer_tg_id = None
            async with aiosqlite.connect(DB_PATH) as db:
                if method == "ton":
                    ton_usd = get_cached_ton_price()
                    ton_amount = round(price / ton_usd, 6) if ton_usd > 0 else 0
                    ton_nano_val = ton_to_nano(ton_amount)
                    comment_code = gen_comment()
                    expires = (datetime.utcnow() + timedelta(minutes=TON_INVOICE_EXPIRY_MIN)).isoformat()
                    cur = await db.execute(
                        "INSERT INTO orders (user_id,stars_amount,price_usd,status,payment_method,"
                        "ton_amount,ton_amount_nano,comment,expires_at,created_at,bot_chat_id,bot_lang,bot_referrer_tg_id,user_id_tg) "
                        "VALUES (?,?,?,'awaiting_payment','ton_transfer',?,?,?,?,?,?,?,?,?)",
                        (target, stars, price, ton_amount, ton_nano_val, comment_code, expires, now, chat_id, lang, bot_referrer_tg_id, tg_id)
                    )
                else:
                    cur = await db.execute(
                        "INSERT INTO orders (user_id,stars_amount,price_usd,status,payment_method,created_at,bot_chat_id,bot_lang,bot_referrer_tg_id,user_id_tg) "
                        "VALUES (?,?,?,'pending',?,?,?,?,?,?)",
                        (target, stars, price, "cryptopay", now, chat_id, lang, bot_referrer_tg_id, tg_id)
                    )
                await db.commit()
            ...
            if method == "cryptopay":
                ...
                bot_return = f"https://t.me/{BOT_USERNAME}?start=paid_{order_id}" if BOT_USERNAME else SITE_URL
                try:
                    params = {
                        "currency_type": "fiat", "fiat": "USD",
                        "amount": f"{price:.2f}",
                        "description": f"⭐ {stars} Stars for {target}"[:1024],
                        "payload": payload_str[:4096],
                        "paid_btn_name": "callback",
                        "paid_btn_url": bot_return,
                        "expires_in": CRYPTOPAY_INVOICE_EXPIRY
                    }
                    if CRYPTOPAY_SWAP_TO_USDT:
                        params["swap_to"] = "USDT"
                    invoice = await asyncio.to_thread(cryptopay_api_call, "createInvoice", params)
                    cp_id = invoice["invoice_id"]
                    pay_url = invoice.get("bot_invoice_url") or invoice.get("pay_url", "")
                    async with aiosqlite.connect(DB_PATH) as db:
                        await db.execute("UPDATE orders SET cryptopay_invoice_id=? WHERE id=?", (cp_id, order_id))
                        await db.commit()
                    kb = {"inline_keyboard": [
                        [{"text": t_bot(lang, "pay_now"), "url": pay_url}],
                        [{"text": t_bot(lang, "btn_check_pay"), "callback_data": f"chk_cp_{order_id}"}],
                        [{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]
                    ]}
                    await edit_tg_message(chat_id, msg_id, f"⭐ {stars} → {escape_md(target)}\n💵 ${price:.2f}", kb)
                except Exception as e:
                    log.error(f"Bot CryptoPay error: {e}")
                    await edit_tg_message(chat_id, msg_id, t_bot(lang, "error_invoice"), bot_menu_keyboard(lang))

            elif method == "ton":
                msg_text = t_bot(lang, "ton_instructions").replace("{price}", str(ton_amount)).replace("{wallet}", TON_WALLET_ADDRESS).replace("{comment}", comment_code)
                kb = {"inline_keyboard": [
                    [{"text": t_bot(lang, "btn_check_pay"), "callback_data": f"chk_ton_{order_id}"}],
                    [{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]
                ]}
                await edit_tg_message(chat_id, msg_id, msg_text, kb)
        ...
        user = await get_bot_user(tg_id, tg_lang, tg_username)
        lang = user["language"]
        state = user["state"]        # أوامر المشرف داخل بوت العملاء (webhook موحد)
        if str(chat_id) == str(os.getenv("OWNER_CHAT_ID", "")) and text.strip().startswith("/admin"):
            await handle_admin_commands(update)
            return {"ok": True}

        if text.startswith("/start"):
            args = text.split(" ", 1)
            # Handle return from CryptoPay payment
            if len(args) > 1 and args[1].startswith("paid_"):
                try:
                    oid = int(args[1].replace("paid_", ""))
                    res = await process_cryptopay_check(oid)
                    order = await get_order_by_id(oid)
                    if res.get("status") == "completed" or (order and order["status"] in ("completed", "paid")):
                        await send_tg_message(chat_id, t_bot(lang, "pay_confirmed").replace("{stars}", str(order["stars_amount"])).replace("{username}", escape_md(order["user_id"])), bot_menu_keyboard(lang))
                    elif res.get("status") == "failed" or (order and order["status"] == "failed"):
                        await send_tg_message(chat_id, t_bot(lang, "pay_failed_bot") if "pay_failed_bot" in BOT_STRINGS.get(lang, {}) else "❌ Order failed. Contact support.", bot_menu_keyboard(lang))
                    else:
                        kb = {"inline_keyboard": [[{"text": t_bot(lang, "btn_check_pay"), "callback_data": f"chk_cp_{oid}"}]]}
                        await send_tg_message(chat_id, t_bot(lang, "pay_pending"), kb)
                except Exception:
                    pass
                return {"ok": True}
            # Handle referral link: /start ref_XXXXXXXX
            if len(args) > 1 and args[1].startswith("ref_"):
                ref_code = args[1].replace("ref_", "", 1).strip()
                if ref_code:
                    referrer = await get_bot_user_by_ref_code(ref_code)
                    if referrer and referrer["tg_id"] != tg_id:
                        linked = await set_bot_referrer(tg_id, referrer["tg_id"])
                        if linked:
                            welcome_ref = {
                                "ar": "🎁 مرحباً! تم تفعيل رابط الإحالة بنجاح. عند شرائك للنجوم، سيحصل المُحيل على عمولة تلقائياً.",
                                "en": "🎁 Welcome! Referral link activated. When you buy stars, your referrer earns a commission automatically.",
                                "ru": "🎁 Добро пожаловать! Реферальная ссылка активирована. При покупке звёзд ваш реферер получает комиссию.",
                                "fa": "🎁 خوش آمدید! لینک ارجاع فعال شد. هنگام خرید ستاره، ارجاع‌دهنده کمیسیون دریافت می‌کند.",
                                "fr": "🎁 Bienvenue ! Lien de parrainage activé.",
                                "de": "🎁 Willkommen! Empfehlungslink aktiviert.",
                                "es": "🎁 ¡Bienvenido! Enlace de referido activado.",
                                "tr": "🎁 Hoş geldiniz! Referans bağlantısı etkinleştirildi.",
                            }
                            await send_tg_message(chat_id, welcome_ref.get(lang, welcome_ref["en"]))
            await set_bot_user_state(tg_id, "idle")
        ...
        if state == "waiting_custom":
        ...
        elif state == "waiting_wd_username":
            target = text.strip()
            if not target.startswith("@"):
                await send_tg_message(chat_id, t_bot(lang, "invalid_username"), {"inline_keyboard": [[{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]]})
                return {"ok": True}
            # اسحب من رصيد بوت المستخدم
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT usd_balance FROM bot_users WHERE tg_id=?", (tg_id,)) as cur:
                    row = await cur.fetchone()
            balance = (row["usd_balance"] if row else 0) or 0
            if balance < 2.0:
                await send_tg_message(chat_id, t_bot(lang, "withdraw_no_balance").replace("{balance}", f"{balance:.2f}"), bot_menu_keyboard(lang))
                await set_bot_user_state(tg_id, "idle")
                return {"ok": True}
            stars_amount = int(balance / 0.015)
            now_iso = datetime.utcnow().isoformat()
            async with aiosqlite.connect(DB_PATH) as db:
                cur = await db.execute(
                    "UPDATE bot_users SET usd_balance = 0 WHERE tg_id=? AND usd_balance >= ?",
                    (tg_id, balance))
                if cur.rowcount == 0:
                    await send_tg_message(chat_id, t_bot(lang, "withdraw_no_balance").replace("{balance}", "0.00"), bot_menu_keyboard(lang))
                    await set_bot_user_state(tg_id, "idle")
                    return {"ok": True}
                await db.execute(
                    ...
                    (-tg_id, stars_amount, target, "stars", "", balance, "pending", now_iso))
                await db.commit()
            await notify_owner(f"💸 سحب من البوت (Stars): {balance}$ ({target}) من tg_id={tg_id}")
            txt = t_bot(lang, "withdraw_ok").replace("{amount}", f"{balance:.2f}").replace("{method}", "⭐ Stars").replace("{target}", escape_md(target))
            await send_tg_message(chat_id, txt, bot_menu_keyboard(lang))
            await set_bot_user_state(tg_id, "idle")

        elif state == "waiting_wd_wallet":
            wallet = text.strip()
            if len(wallet) < 10:
                await send_tg_message(chat_id, t_bot(lang, "enter_wd_wallet"), {"inline_keyboard": [[{"text": t_bot(lang, "cancel"), "callback_data": "cancel"}]]})
                return {"ok": True}
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT usd_balance FROM bot_users WHERE tg_id=?", (tg_id,)) as cur:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                await db.execute(
                    "INSERT INTO withdrawals (user_id,stars_amount,recipient_username,method,wallet_address,usd_amount,status,created_at) VALUES (?,?,?,?,?,?,?,?)",
                    (-tg_id, 0, "", "ton", wallet, balance, "pending", now_iso))
                await db.commit()
            await notify_owner(f"💸 سحب من البوت (TON): {balance}$ ({wallet}) من tg_id={tg_id}")
            txt = t_bot(lang, "withdraw_ok").replace("{amount}", f"{balance:.2f}").replace("{method}", "💎 TON").replace("{target}", escape_md(wallet))
            await send_tg_message(chat_id, txt, bot_menu_keyboard(lang))
            await set_bot_user_state(tg_id, "idle")

        else:
            # أي رسالة نصية خارج خطوات الشراء (حالة idle): أعد عرض القائمة بدلاً من الصمت التام
...
async def get_me(user: dict = Depends(get_current_user)):
    ...
    # نظام الإحالات صار قائماً على البوت — نُعيد رابط البوت هنا
    bot_ref_link = None
    bot_ref_stats = {"ref_count": 0, "usd_balance": 0.0, "total_earned": 0.0}
    tg_raw = (user.get("telegram_id") or "").lstrip("@").strip()
    if tg_raw and BOT_USERNAME:
        # حاول ربط حساب الموقع بمستخدم بوت عبر tg username (فقط للعرض)
        try:
            async with aiosqlite.connect(DB_PATH) as db:
                db.row_factory = aiosqlite.Row
                # لا يوجد ربط مباشر عبر username في bot_users؛ نُنشئ فقط رابط عام
                pass
        except Exception:
            pass
    if BOT_USERNAME:
        # رابط عام يعرض للمستخدم — بعد أول تفاعل مع البوت سيحصل على رمز إحالته الشخصي
        bot_ref_link = f"https://t.me/{BOT_USERNAME}"
    return {
        "id": user["id"], "email": user["email"],
        "referral_code": user["referral_code"],
        "referral_url": f"{SITE_URL}/?ref={user['referral_code']}",
        "bot_username": BOT_USERNAME,
        "bot_referral_url": bot_ref_link,
...
# ═══════════════════════════════════════════════════════════════════════════
# Core Payment Checking Functions
# ═══════════════════════════════════════════════════════════════════════════

async def _notify_bot_customer_pay_confirmed(order: dict):
    """تنبيه العميل عبر البوت أن الدفع تم التحقق منه (قبل محاولة تسليم النجوم)."""
    chat_id = order.get("bot_chat_id")
    if not chat_id:
        return
    lang = order.get("bot_lang") or "en"
    txt = (t_bot(lang, "pay_confirmed")
           .replace("{stars}", str(order.get("stars_amount", 0)))
           .replace("{username}", escape_md(order.get("user_id", "") or "")))
    # إضافة سطر تنبيه واضح أن النجوم جارٍ إرسالها
    delivery_note = {
        "ar": "\n\n⏳ جارٍ إرسال النجوم إلى حسابك...",
        "en": "\n\n⏳ Delivering stars to your account...",
        "ru": "\n\n⏳ Отправка звёзд на ваш аккаунт...",
        "fa": "\n\n⏳ در حال ارسال ستاره‌ها به حساب شما...",
        "fr": "\n\n⏳ Envoi des étoiles à votre compte...",
        "de": "\n\n⏳ Sterne werden an Ihr Konto gesendet...",
        "es": "\n\n⏳ Enviando estrellas a tu cuenta...",
        "tr": "\n\n⏳ Yıldızlar hesabınıza gönderiliyor...",
    }.get(lang, "\n\n⏳ Delivering stars to your account...")
    try:
        await send_tg_message(int(chat_id), txt + delivery_note, bot_menu_keyboard(lang))
    except Exception as e:
        log.warning(f"notify_bot_customer_pay_confirmed error: {e}")

async def _notify_bot_customer_delivery_failed(order: dict):
    """إذا فشل شراء النجوم بعد تأكيد الدفع (نقص رصيد في محفظة المشروع مثلاً)،
    نُبلّغ العميل بذلك ونطلب التواصل مع الدعم @Tt09x."""
    chat_id = order.get("bot_chat_id")
    if not chat_id:
        return
    lang = order.get("bot_lang") or "en"
    msg_map = {
        "ar": ("❌ *فشل إرسال النجوم*\n\n"
               "تم تأكيد دفعك بنجاح ✅ لكن حدث خلل تقني أثناء إرسال النجوم إلى حسابك.\n"
               "لا تقلق — أموالك محفوظة ✋\n\n"
               f"📞 يرجى التواصل مع الدعم فوراً: {SUPPORT_USERNAME}\n"
               f"🔢 رقم الطلب: `#{order.get('id')}`"),
        "en": ("❌ *Stars delivery failed*\n\n"
               "Your payment was confirmed successfully ✅ but a technical issue occurred while sending the stars to your account.\n"
               "Don't worry — your funds are safe ✋\n\n"
               f"📞 Please contact support immediately: {SUPPORT_USERNAME}\n"
               f"🔢 Order ID: `#{order.get('id')}`"),
        "ru": ("❌ *Не удалось отправить звёзды*\n\n"
               "Ваш платёж подтверждён ✅, но при отправке звёзд возникла техническая проблема.\n"
               "Не беспокойтесь — ваши средства сохранены ✋\n\n"
               f"📞 Немедленно обратитесь в поддержку: {SUPPORT_USERNAME}\n"
               f"🔢 № заказа: `#{order.get('id')}`"),
        "fa": ("❌ *ارسال ستاره‌ها ناموفق بود*\n\n"
               "پرداخت شما با موفقیت تأیید شد ✅ اما هنگام ارسال ستاره‌ها به حساب شما مشکلی رخ داد.\n"
               "نگران نباشید — پول شما محفوظ است ✋\n\n"
               f"📞 لطفاً فوراً با پشتیبانی تماس بگیرید: {SUPPORT_USERNAME}\n"
               f"🔢 شماره سفارش: `#{order.get('id')}`"),
        "fr": ("❌ *Échec de la livraison des étoiles*\n\n"
               "Votre paiement a été confirmé ✅ mais un problème technique est survenu.\n"
               "Ne vous inquiétez pas — vos fonds sont en sécurité ✋\n\n"
               f"📞 Contactez le support immédiatement : {SUPPORT_USERNAME}\n"
               f"🔢 Commande : `#{order.get('id')}`"),
        "de": ("❌ *Sternenlieferung fehlgeschlagen*\n\n"
               "Ihre Zahlung wurde bestätigt ✅, aber beim Senden der Sterne ist ein technisches Problem aufgetreten.\n"
               "Keine Sorge — Ihr Geld ist sicher ✋\n\n"
               f"📞 Bitte kontaktieren Sie sofort den Support: {SUPPORT_USERNAME}\n"
               f"🔢 Bestellung: `#{order.get('id')}`"),
        "es": ("❌ *Error al enviar estrellas*\n\n"
               "Tu pago fue confirmado ✅ pero ocurrió un problema técnico al enviar las estrellas.\n"
               "No te preocupes — tus fondos están seguros ✋\n\n"
               f"📞 Contacta al soporte inmediatamente: {SUPPORT_USERNAME}\n"
               f"🔢 Pedido: `#{order.get('id')}`"),
        "tr": ("❌ *Yıldız gönderimi başarısız*\n\n"
               "Ödemeniz onaylandı ✅ ancak yıldızları hesabınıza gönderirken teknik bir sorun oluştu.\n"
               "Endişelenmeyin — paranız güvende ✋\n\n"
               f"📞 Lütfen destek ile hemen iletişime geçin: {SUPPORT_USERNAME}\n"
               f"🔢 Sipariş: `#{order.get('id')}`"),
    }
    text = msg_map.get(lang, msg_map["en"])
    kb = {"inline_keyboard": [
        [{"text": "📞 " + ("تواصل مع الدعم" if lang == "ar" else "Contact Support"), "url": f"https://t.me/{SUPPORT_USERNAME.lstrip('@')}"}],
        [{"text": t_bot(lang, "btn_open_app") or "🛒", "web_app": {"url": SITE_URL}}]
    ]}
    try:
        await send_tg_message(int(chat_id), text, kb)
    except Exception as e:
        log.warning(f"notify_bot_customer_delivery_failed error: {e}")


async def process_cryptopay_check(order_id: int) -> dict:
    ...
    try:
        ...
        log.error(f"cryptopay_get_invoice failed for order #{order_id}: {e}")
    ...
    log.info(f"🔍 CryptoPay order #{order_id}: invoice status = {inv_status}")
    ...
    # ✅ الدفع مؤكّد → علِّم الطلب "paid" وأرسل تنبيهاً فورياً للعميل عبر البوت
    await set_order_status(order["id"], "paid")
    order = await get_order_by_id(order["id"])  # refresh
    await _notify_bot_customer_pay_confirmed(order)
    await send_success_alert(order)  # للمالك
    
    # الآن نحاول تسليم النجوم عبر Fragment
    ...
    if success:
        ...
        await process_order_referral(updated or order)
        return {"success": True, "status": "completed"}
    else:
        # فشل تسليم النجوم (رصيد المحفظة/Fragment) → أبلغ العميل بالتواصل مع الدعم
        await _notify_bot_customer_delivery_failed(order)
        await send_failure_alert(order)
        return {"success": False, "status": "failed", "delivery_failed": True}

async def process_ton_check(order_id: int) -> dict:
    order = await get_order_by_id(order_id)
    if not order:
        return {"success": False, "status": "not_found"}
    if order["status"] in ("completed", "paid"):
    ...
    # ✅ الدفع مؤكّد → علِّم الطلب "paid" وأبلغ العميل عبر البوت
    await set_order_status(order["id"], "paid")
    order = await get_order_by_id(order["id"])
    await _notify_bot_customer_pay_confirmed(order)
    await send_ton_success_alert(order, ton_rcvd)  # تنبيه المالك
    ...
    if success:
        ...
        # فشل التسليم — أبلغ العميل بالتواصل مع الدعم
        await _notify_bot_customer_delivery_failed(order)
        await send_failure_alert(order)
        return {"success": False, "status": "failed", "delivery_failed": True}
...
async def create_cryptopay_invoice(request: Request, req: CryptoPayInvoiceRequest, user: Optional[dict] = Depends(get_optional_user)):
...
async def check_cryptopay_payment(request: Request, req: CheckTonRequest):
...
@limiter.limit("10/minute")
async def create_ton_order(request: Request, req: TonOrderRequest, user: Optional[dict] = Depends(get_optional_user)):
    if not req.user_id.startswith("@") or len(req.user_id) < 2:
        raise HTTPException(400, "المعرف يجب أن يبدأ بـ @")
    if req.stars_amount < MIN_STARS:
        raise HTTPException(400, f"الحد الأدنى {MIN_STARS} نجوم")
...
@limiter.limit("30/minute")
async def check_ton_payment(request: Request, req: CheckTonRequest):
    res = await process_ton_check(req.order_id)
...
async def cryptomus_webhook(request: Request):
    ...
    if not order or order["status"] in ("completed", "paid"):
        return {"ok": True}
    # ✅ الدفع مؤكّد → علِّم "paid" وأبلغ العميل
    await set_order_status(order["id"], "paid")
    order = await get_order_by_id(order["id"])
    await _notify_bot_customer_pay_confirmed(order)
    await send_success_alert(order)
...
async def cryptopay_webhook(secret: str, request: Request):
    ...
    # ✅ الدفع مؤكّد من webhook → علِّم "paid" وأبلغ العميل فوراً
    await set_order_status(order["id"], "paid")
    order = await get_order_by_id(order["id"])
    await _notify_bot_customer_pay_confirmed(order)
    await send_success_alert(order)
...
async def request_withdrawal(req: WithdrawalRequest, user: dict = Depends(get_current_user)):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("UPDATE users SET usd_balance = usd_balance - ? WHERE id=? AND usd_balance >= ?",
                         (req.amount_usd, user["id"], req.amount_usd))
        if cur.rowcount == 0:
...
async def public_config():
    return {
        ...
        "price_per_star": GLOBAL_CONFIG["PRICE_PER_STAR"],
        "bot_username": BOT_USERNAME,
        "commission_percent": float(GLOBAL_CONFIG.get("COMMISSION_PERCENT", 10.0)),
        "support_username": SUPPORT_USERNAME.lstrip("@"),
    }

@app.get("/api/referrals")
async def user_referrals(user: dict = Depends(get_current_user)):
    """يُعيد سجل الإحالات لهذا المستخدم — من الموقع (المسار القديم) + من البوت إن كان معرّف تيليجرام مرتبط."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        # الإحالات من الموقع (referrer_code == user's code)
        async with db.execute("""
            SELECT re.stars_amount, re.usd_amount, re.created_at,
                   o.user_id AS buyer, o.stars_amount AS order_stars, o.price_usd
...
async def user_withdrawals(user: dict = Depends(get_current_user)):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("""
            SELECT id, stars_amount, recipient_username, method, wallet_address, usd_amount, status, created_at
...
async def admin_login_api(request: Request):
    ...
    if secret_key:
        ...
        log.warning("RECAPTCHA_SECRET not set - captcha skipped (rate limiting still active)")
            
    if email == os.getenv("ADMIN_EMAIL", "admin@stars-hub.com") and password == os.getenv("ADMIN_PASSWORD", "strongpassword123"):
        token = jwt.encode({"role": "admin", "exp": datetime.utcnow() + timedelta(hours=2)}, ADMIN_SECRET, algorithm="HS256")
        return {"token": token}
...
async def admin_update_commission(req: Request):
    ...
    try:
        ...
        raise HTTPException(400, "قيمة العمولة غير صالحة")
    if new_comm < 0.0 or new_comm > 100.0:
        raise HTTPException(400, "العمولة يجب أن تكون بين 0% و 100%")
...
async def admin_get_commission(req: Request):
    await verify_admin(req)
    return {"commission": float(GLOBAL_CONFIG.get("COMMISSION_PERCENT", 10.0))}
...
async def admin_referrals_data(req: Request, secret: str = ""):
    if secret and hmac.compare_digest(secret, ADMIN_SECRET):
        pass
    else:
        await verify_admin(req)
    async with aiosqlite.connect(DB_PATH) as db:
