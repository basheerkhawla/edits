    ...
    if "callback_query" in update:
        ...
        # ── Check payment status callbacks ──
            ...
            if res.get("status") == "completed" or (order and order["status"] in ("completed", "paid")):
                ...
                await edit_tg_message(chat_id, msg_id, t_bot(lang, "pay_confirmed").replace("{stars}", str(order["stars_amount"])).replace("{username}", order["user_id"]))
        ...
        # ── Payment methods ──
            ...
            if method == "cryptopay":
                ...
                try:
                    ...
                    await edit_tg_message(chat_id, msg_id, f"⭐ {stars} → {target}\n💵 ${price:.2f}", kb)
                except Exception as e:
        ...
        if text.startswith("/start"):
            ...
            if len(args) > 1 and args[1].startswith("paid_"):
                try:
                    ...
                    order = await get_order_by_id(oid)
                    if res.get("status") == "completed" or (order and order["status"] in ("completed", "paid")):
                        await send_tg_message(chat_id, t_bot(lang, "pay_confirmed").replace("{stars}", str(order["stars_amount"])).replace("{username}", order["user_id"]), bot_menu_keyboard(lang))
        ...
        if state == "waiting_custom":
            ...
            txt = t_bot(lang, "withdraw_ok").replace("{amount}", f"{balance:.2f}").replace("{method}", "⭐ Stars").replace("{target}", target)
            ...
            txt = t_bot(lang, "withdraw_ok").replace("{amount}", f"{balance:.2f}").replace("{method}", "💎 TON").replace("{target}", wallet)
            await send_tg_message(chat_id, txt, bot_menu_keyboard(lang))
            await set_bot_user_state(tg_id, "idle")
