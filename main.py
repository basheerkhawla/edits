...
async def answer_callback(callback_id: str, text: str = None, show_alert: bool = False):
    ...
    try:
        await asyncio.to_thread(req_lib.post, f"https://api.telegram.org/bot{tg_token}/answerCallbackQuery", json=payload, timeout=5)
    except Exception as e:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                if method == "ton":
                    ...
                    cur = await db.execute(
                        ...
                        (target, stars, price, ton_amount, ton_nano_val, comment_code, expires, now)
                    )
                else:
                    cur = await db.execute(
                        "INSERT INTO orders (user_id,stars_amount,price_usd,status,payment_method,created_at) "
                        "VALUES (?,?,?,'pending',?,?)",
                        (target, stars, price, "cryptopay", now)
                    )
                await db.commit()
...
async def admin_login_api(request: Request):
    ...
    secret_key = os.getenv("RECAPTCHA_SECRET", "6Lfx020tAAAAABnlMtH9YIW6hPoz03Ic1aMgY4_d")
